...
static void run_ecmult_multi_bench(bench_data* data, size_t count, int includes_g, int num_iters) {
    ...
    /* Run the benchmark. */
    if (includes_g) {
        snprintf(str, 32, "ecmult_multi_%ip_g", (int)count - 1);
    } else {
        snprintf(str, 32, "ecmult_multi_%ip", (int)count);
    }
