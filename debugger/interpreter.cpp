...
,   m_applied_tweak{false} {
    ...
    m_k = (CHashWriter(HasherTapLeaf) << uint8_t(control[0] & TAPROOT_LEAF_MASK) << script).GetSHA256();
...
TaprootCommitmentEnv::State TaprootCommitmentEnv::Iterate() {
    ...
    if (m_i < m_path_len) {
        ...
        auto node_begin = m_control.data() + TAPROOT_CONTROL_BASE_SIZE + TAPROOT_CONTROL_NODE_SIZE * m_i;
        if (std::lexicographical_compare(m_k.begin(), m_k.end(), node_begin, node_begin + TAPROOT_CONTROL_NODE_SIZE)) {
            btc_taproot_logf("  - %d: node = %02x...; taproot control node match -> k first\n", m_i, node_begin[0]);
            m_k_desc = strprintf("TapBranch(%s || Span<%d,%zu>=%s)", m_k_desc.c_str(), TAPROOT_CONTROL_BASE_SIZE + TAPROOT_CONTROL_NODE_SIZE * m_i, TAPROOT_CONTROL_NODE_SIZE, HexStr(Span<const unsigned char>(node_begin, TAPROOT_CONTROL_NODE_SIZE)).c_str());
            ss_branch << m_k << Span<const unsigned char>(node_begin, TAPROOT_CONTROL_NODE_SIZE);
        } else {
            btc_taproot_logf("  - %d: node = %02x...; taproot control node mismatch -> k second\n", m_i, node_begin[0]);
            m_k_desc = strprintf("TapBranch(Span<%d,%zu>=%s || %s)", TAPROOT_CONTROL_BASE_SIZE + TAPROOT_CONTROL_NODE_SIZE * m_i, TAPROOT_CONTROL_NODE_SIZE, HexStr(Span<const unsigned char>(node_begin, TAPROOT_CONTROL_NODE_SIZE)).c_str(), m_k_desc.c_str());
            ss_branch << Span<const unsigned char>(node_begin, TAPROOT_CONTROL_NODE_SIZE) << m_k;
        }
        btc_taproot_logf("  (%s)\n", m_k_desc.c_str());
    ...
    if (!m_applied_tweak) {
        ...
        m_k = (CHashWriter(HasherTapTweak) << MakeSpan(m_p) << m_k).GetSHA256();
